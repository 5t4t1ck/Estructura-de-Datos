...
```         
library(igraph)  ciudades <- c("Quito", "Guayaquil", "Cuenca", "Ambato", "Manta") rutas <- c("Quito", "Guayaquil", "Guayaquil", "Cuenca", "Quito", "Ambato", "Guayaquil", "Manta") grafo <- graph(rutas, vertices = ciudades) 
```
...
```         
existe_conexion <- are.connected(grafo, "Quito", "Guayaquil") 
```
...
```         
grafo <- add.vertices(grafo, 1, name = "Loja") grafo <- add.edges(grafo, c("Loja", "Cuenca", "Loja", "Guayaquil")) 
```
...
```         
grafo <- delete.vertices(grafo, "Manta") 
```
...
```         
camino_mas_corto <- shortest_paths(grafo, "Quito", "Cuenca") 
```
...
```         
longitud_camino <- graph_distance(grafo, "Quito", "Cuenca") 
```
...
```         
caminos <- all_simple_paths(grafo, "Quito", "Cuenca") 
```
...
```         
num_componentes <- clusters(grafo)$no 
```
...
```         
componente_conexa <- components(grafo)$membership[which(V(grafo)$name == "Quito")] ciudades_componente <- names(V(grafo))[which(components(grafo)$membership == componente_conexa)] 
```
...
```         
subgrafo <- induced_subgraph(grafo, which(components(grafo)$membership == 1)) 
```
...
```         
plot(grafo, vertex.label = V(grafo)$name, edge.arrow.size = 0.5) 
```
